...
- **/books/:id**
  - 書籍詳細画面。
  - 構成：
    - 上部：書籍の基本情報と編集フォーム（タイトル／総ページ数／著者／出版社／ISBN／状態・進捗）
    - 左側：読書ログセクション
      - クイック更新フォーム（累計ページ・時間・短いメモ）
      - ログ一覧（新しい順）：「日付・＋Nページ・累計ページ・時間・メモ」
      - 「直前ログのUndo」ボタン（最新1件のみ対象）
    - 右側：メモ（Notes）セクション
      - メモ一覧（新しい順）
      - メモ追加フォーム
      - メモ編集／削除
  - 読み取り専用ユーザーの場合：
    - 編集フォーム・クイック更新・メモ追加／編集／削除ボタンを無効化。
    - 必要に応じて「読み取り専用です」メッセージを表示。
