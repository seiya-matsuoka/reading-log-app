...
- **サーバ側（バックエンド）が最終的な正**（フロントはUX改善のための事前チェック）。
- 文字列は **リンクを含む入力を禁止**（`http(s)://` / `www.` などの検出）。
- 数値は「文字列入力→数値化→範囲チェック」を行う。
- ReadOnly ユーザーは **書き込み系HTTPメソッド（POST/PUT/PATCH/DELETE）をサーバ側で拒否**。
  - フロントでも送信ボタン抑止・説明表示を行う。

---

### 4.2 サーバ側（バックエンド）

#### 4.2.1 共通（パース・不正リクエスト）

- JSON パース失敗（SyntaxError / entity.parse.failed）は **400**。
- ルーティング未該当は **404**（JSONで返却）。
- DB制約違反は **409**。
- それ以外の未捕捉例外は **500**。
...
- `author` / `publisher`
  - 任意
  - リンク禁止
  - 更新で空文字を許容する場合は Service 側で `null` 扱いにしてクリア可能
- `isbn`
  - 任意
  - 数字のみ / 桁数は 10桁 or 13桁
- 更新（PATCH）
  - 「未指定（undefined）は現状維持」を前提に更新入力を整形して更新

#### 4.2.3 Logs（追加／直近削除）

- `bookId`（params）
  - 数値（整数）必須、変換できなければ 400
- `cumulative_pages`
  - 0以上
  - `total_pages` 以下
  - 直前の `pages_read` からの差分が
    - 0 → NG（「増分が0」）
    - マイナス → NG（「逆行」）
- `minutes`
  - 0以上の整数
- `date_jst`
  - `YYYY-MM-DD` 形式
  - 未来日NG
  - 未指定の場合は SQL 側で「JSTの当日」が採用される
- `memo`
  - 任意
  - 500文字以内
  - リンク禁止
- 直近削除（Undo）
  - 最新ログが存在しない場合は NotFound（404）
  - 削除後に `books.pages_read / minutes_total` を再計算して更新
...
- Render のコールドスタート（初回起動遅延）を前提に、
  - 通常のローディング表示に加え、
  - **一定時間（閾値）を超えたら「起動待ちの可能性」を明示**して離脱を抑える。
- 画面別に
  - **ページレベル**（一覧/詳細の初期表示）
  - **パーツレベル**（StatsBar / Notes 等）
  のローディングを使い分ける。
...
- NotFound → 404
- それ以外 → 500
- 404（ルート未該当）も JSON で返す

#### 5.3.2 フロントの API ラッパ（api.js）

- `fetch` 失敗（ネットワーク）は status=0 として Error を throw
- レスポンスの JSON から `message` を抽出して「直近 message」として保持
- `res.ok` でない場合は Error を throw
  - `e.status` / `e.payload` を付与（呼び出し側で分岐可能）

#### 5.3.3 エラー表示の使い分け

- ページ初期表示の失敗（BooksList / BookDetail）
  - 画面内にエラーパネルを表示
- 操作系（保存・削除など）の失敗
  - Toast（error）で通知
- 表示メッセージの基本優先順位：
  1. サーバから返った `message`（最優先）
  2. フロント側で定義した汎用メッセージ（ネットワーク等）
  3. それも無ければフォールバック文言
...
- PageLoading の slow 表示により、
  - 「壊れている」のではなく「サーバ起動待ち」である可能性を明示
  - 待機継続を促す
- しきい値は 5秒（必要に応じて調整可能）
