...
- **レスポンスフォーマット**
  - JSON を基本とし、成功時は以下のような構造を基本とする：
    - `200 OK: { data: ..., message?: string }`
    - `201 Created: { data: ..., message?: string }`
    - `204 No Content: (レスポンスボディなし)`
  - `message` はバックエンド側の `MSG` から選択した **テキスト形式** であり、保存・更新・削除・Undo などの結果メッセージを表す。  
    フロントエンドは共通ラッパ（`api.js`）で最後に受け取った `message` を保持し、
    成功時には `api.getLastMessage()` を通じてトーストなどに表示する。
  - エラー時は HTTP ステータスコードに加え、以下のような構造を基本とする：
    - `4xx/5xx: { error: true, message: string }`
  - フロントエンド側では、`res.ok` ではない場合、または `json.error === true` の場合をエラーとして扱い、
    この `message` を `Error` オブジェクトの `message` プロパティとしてスローする。
  - バックエンドから返る `message` がそのままユーザー向けメッセージとなり、
    汎用的なネットワークエラー文言や UI 固有ラベルはフロントエンド側の`MSG.FE.*` で別途管理する。
