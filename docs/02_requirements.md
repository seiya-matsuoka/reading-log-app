...
- **構成方針**（分離構成）
  ...
  - フロントとバックエンドは別オリジンとし、**CORS 設定で正規のオリジンのみ許可**する。

- **SPA ルーティング**
  - Vercel 側の設定で、直接アクセスされたパス（例：`/books/xxx`）でも `index.html` を返す。
  - これにより、SPA でのクライアントサイドルーティングが機能する。

- **環境分離**
  - 開発／本番で DB 接続文字列やオリジンを分離する。
  - Render・Vercel の環境変数として設定し、リポジトリには `.env.example` のみ含める。
